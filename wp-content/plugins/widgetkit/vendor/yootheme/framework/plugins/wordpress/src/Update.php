...
class Update
{
    ...
    public function __construct()
    {
        ...
        add_filter(
            'plugins_api',
            function ($result, $action, $args) use ($self) {
                return $action == 'plugin_information' && isset($self->plugins[$args->slug])
                    ? $self->fetchData($self->plugins[$args->slug])
                    : false;
            },
            10,
            3
    ...
    public function register($name, $type, $remote, array $options = [])
    {
        ...
        if (isset($options['expiration'])) {
            ...
            if (
                $type == 'plugin' and
                ($transient = get_site_transient('update_plugins')) and
            ...
            if (
                $type == 'theme' and
                ($transient = get_site_transient('update_themes')) and
                isset($transient->response[$options['id']], $transient->last_checked)
            ) {
                if (time() - $transient->last_checked > $timeout) {
    ...
    public function prepare($transient, array $update)
    {
        if (
            ...
            ($data = $transient->response[$update['id']])
        ) {
            if (isset($update['key'])) {
    ...
    {
        if (
            isset($transient->checked[$update['id']]) and
            ($data = $this->fetchData($update)) and
            version_compare($transient->checked[$update['id']], $data->version, '<')
        ) {
    ...
    public function fetchData(array $update)
    {
        ...
        if (
            ($response = wp_remote_retrieve_body(wp_remote_get($remote))) and
