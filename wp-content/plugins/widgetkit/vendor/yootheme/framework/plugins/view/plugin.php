    ...
    'main' => function ($app, $self) {
        ...
        $app->on(
            ...
            function ($event, $app) {
                ...
                $app['scripts']->register(
                    'vue-config',
                    $path . 'plugins/view/lib/vue-config.min.js',
                    ['vue', 'config']
                );
                $app['scripts']->register(
                    'vue-resource',
                    $path . 'assets/vue-resource/dist/vue-resource.min.js',
                    'vue-config'
                );
                $app['scripts']->register(
                    'vue-translator',
                    $path . 'plugins/view/lib/vue-translator.min.js',
                    'vue-config'
                );
            },
            10
        ...
        $app->on(
            ...
            function ($event, $app) use ($self) {
                ...
                $app['scripts']->register(
                    'config',
                    sprintf('var $config = %s;', json_encode($config)),
                    [],
                    'string'
                );
            },
