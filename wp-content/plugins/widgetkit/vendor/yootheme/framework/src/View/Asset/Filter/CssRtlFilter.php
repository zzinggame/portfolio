...
class CssRtlFilter implements FilterInterface
{
    ...
    public function process($css)
    {
        ...
        $css = preg_replace_callback(
            ...
            function ($arr) use ($that) {
                ...
                if (strpos($val, '!important') === false && $isImportant) {
                    $val .= '!important';
                }
    ...
    {
        if (preg_match('/left/', $v)) {
            return 'right';
        }
        if (preg_match('/right/', $v)) {
            return 'left';
        }
        return $v;
    }
    ...
    public function background($v)
    {
        ...
        $parseSingle = function ($v) use ($that) {
            ...
            // background-position
            $v = preg_replace_callback(
                '/\s(left|right|center|top|bottom|-?\d+([a-zA-Z]{2}|%?))\s(left|right|center|top|bottom|-?\d+([a-zA-Z]{2}|%?))[;\s]?/i',
                function ($arr) use ($that) {
                    return ' ' .
                        $that->bgPosition(trim($arr[0], ';')) .
                        (strpos($arr[0], ';') !== false ? ';' : ' ');
                },
                $v
            );
