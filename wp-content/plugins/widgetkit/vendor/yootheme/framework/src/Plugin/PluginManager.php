...
class PluginManager implements \ArrayAccess, \IteratorAggregate
{
    ...
    protected function loadPlugin($name, $plugin)
    {
        ...
        $class = is_string($plugin['main'])
            ? $plugin['main']
            : 'YOOtheme\\Framework\\Plugin\\Plugin';
     ...
     */
    protected function resolvePlugins($plugin, &$resolved = [], &$unresolved = [])
    {
                ...
                if (!isset($resolved[$required])) {
                    if (isset($unresolved[$required])) {
                        throw new \RuntimeException(
                            sprintf(
                                'Circular requirement "%s > %s" detected.',
                                $plugin['name'],
                                $required
                            )
                        );
    ...
    protected function resolvePath($plugin, $path)
    {
        ...
        if (
            !(
                $path[0] == '/' ||
                (strlen($path) > 3 && ctype_alpha($path[0]) && $path[1] == ':' && $path[2] == '/')
            )
        ) {
